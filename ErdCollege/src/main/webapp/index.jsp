...
<html>
	...
	<body>
		<h3>미니 학사관리 프로젝트</h3>
		<c:if test="${sessionScope.sessStudent == null}">
		...
		<form action="/ErdCollege/student/login.do" method="post">
			<table border="1">
				<tr>
					<td>학번</td>
					<td><input type=text" name="stdNo" placeholder="학번을 입력해주세요"/></td>
				</tr>
				<tr>
					<td>이름</td>
					<td><input type=text" name="stdName" placeholder="이름을 입력해주세요"/></td>
				</tr>
				<tr>
		...
		<c:if test="${not empty sessionScope.sessStudent}">
			<p>
				${sessionScope.sessStudent.stdName}(${sessionScope.sessStudent.stdNo})님 반갑습니다. 서비스를 이용하시기 바랍니다.
